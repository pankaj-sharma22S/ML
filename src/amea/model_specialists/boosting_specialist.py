"""Gradient Boosting Model Specialist Agent."""
...
class BoostingSpecialistAgent(ModelSpecialistBase):
    """Specialist managing sequential gradient boosted decision trees."""
    ...
    def capabilities(self) -> ModelCapability:
        return ModelCapability(
            ...
            strengths=["State-of-the-art tabular accuracy", "Native histogram binning", "Handles non-linear feature interactions", "Fast inference"],
            limitations=["Hyperparameter sensitive", "Requires tuning of learning rate and early stopping"],
            requires_scaling=False,
            handles_missing_values_natively=True,
    ...
    def validate_experiment(
        ...
        if task_spec.task_type not in caps.supported_task_types:
            return SpecialistValidationResult(
                is_compatible=False,
                status="INCOMPATIBLE",
                rejection_reason=f"BoostingSpecialist does not support task type '{task_spec.task_type.value}'.",
    ...
    def prepare_execution(
        ...
        model_cls = "HistGradientBoostingClassifier" if is_classification else "HistGradientBoostingRegressor"
