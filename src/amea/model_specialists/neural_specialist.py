"""Neural Network (MLP) Model Specialist Agent."""
...
class NeuralSpecialistAgent(ModelSpecialistBase):
    """Specialist managing feedforward multi-layer perceptron neural networks."""

    def capabilities(self) -> ModelCapability:
        return ModelCapability(
            model_family=ModelFamily.TABULAR_NEURAL_NET,
            supported_task_types=[TaskType.BINARY_CLASSIFICATION, TaskType.MULTICLASS_CLASSIFICATION, TaskType.REGRESSION],
            strengths=["Universal function approximation", "Continuous representation learning", "Non-linear feature cross-embeddings"],
            limitations=["Sample inefficient for small datasets (< 5,000 rows)", "Requires careful scaling and normalization"],
    ...
    def validate_experiment(
        ...
        if task_spec.task_type not in caps.supported_task_types:
            return SpecialistValidationResult(
                is_compatible=False,
                status="INCOMPATIBLE",
                rejection_reason=f"NeuralSpecialist does not support task type '{task_spec.task_type.value}'.",
...
# 2. Impute and Scale (Strictly required for Neural Nets)
